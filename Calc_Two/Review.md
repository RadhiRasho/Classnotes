...
The disk method is used to find the volume of a solid formed by rotating a region enclosed by two functions around a given axis. To find the volume using the disk method, you need to integrate the area of the cross-sections over the interval.

The volume of a solid generated by revolving the region between $\large f(x)$ and $\large g(x)$ from $\large a$ to $\large b$ around the $\large y$-axis using the disk method is given by:
...
The volume of a solid generated by revolving the region between $\large f(x)$ and $\large g(x)$ from $\large a$ to $\large b$ around the $\large x$-axis using the disk method is given by:
